...
public class MeasurementEditFactoryController
...

	{
		...
		generalProperties.add(new PropertyEntry("Start Date", textFieldDate));
	...
	protected void setEditTemplate(Measurement item) {
		...
		textFieldDate.setText(item.getStartDate().toString());
		
		
//		super.selectComboBoxItem(comboBoxDateFormat,
//			item.getDateFormat());
	}
	...
	public Measurement createItem() {
		...
		Date date = new Date(textFieldDate.getText());
