...
public class PhantomUI implements UIEntity<Phantom> {
	...
	public List<AbstractColumn<Phantom, ?>> createColumns() {
		...
		columns.add(new FilterColumn<Phantom, String>("Manufacturing",
			item -> item.getManufacturing(), path -> path.getName(),
		...
		ColumnUtils.createCountingColumn("Measurements", columns,
	...
	private class PhantomEditFactoryController
...

		{
			...
			generalProperties
				.add(new PropertyEntry("Literarure Base", comboBoxLiterature));
			...
			generalProperties
				.add(new PropertyEntry("Manufacturing", comboBoxManufacturing));
			generalProperties
				.add(new PropertyEntry("Nominal thickness", textFieldThickness));
		...
		private void createComboBoxes() {
			...
			createComboBox(comboBoxFabricationType, FabricationType.class,
				f -> f.getValue(), item -> updateId());
